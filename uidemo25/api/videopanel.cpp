                       ...
                       DWORD dwStreamType, void * pUser)
{
    ...
    //QLOG_INFO()<<"CALLBACK::NewFrame start";
    VideoWidget *pVideoPanel = (VideoWidget *)pUser;
    pVideoPanel->newFrameEmit(intCameraTemperature, dwCamState);
}
...
void VideoPanel::play(VideoWidget * widget)
{
    QLOG_INFO()<<"VideoPanel::play start";
    CMagDevice &dev = widget->m_magDev;
    const struct_CamInfo *camInfo = dev.GetCamInfo();
    ...
    if (!dev.StartProcessImage(&paraOut, NewFrame, streamType,
                               (void *)widget)) {
        QLOG_INFO()<<"VideoPanel::play fail";
        return;
    }
    dev.SetColorPalette(IronBow);
    QLOG_INFO()<<"VideoPanel::play end";
}
...
void VideoPanel::change_video(int index, int flag)
{
    ...
    for (int i = 0; i < videoCount; i++) {
        if (i >= index) {
            qDebug()<<"change_video"<<row<<column;

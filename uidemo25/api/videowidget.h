...
#include <QMouseEvent>
#include <QPoint>
#include "api/camera/MagDevice.h"
...
public:
    ...
    int getMaxTem(){return maxTem;}
    bool linkCamera(QString ip);
    CMagDevice m_magDev;
protected:
...
public slots:
    ...
    //接收相机拉流数据 cgt
   void process(int intCameraTemperature);
signals:
    //播放成功
    ...
    void mousePressed(); //获取鼠标位置 cgt
    //拉流数据帧 cgt
    void newFrameComing(int intCameraTemperature);
...
#endif // VIDEOWIDGET_H
/*
void CThermoGroupSampleDlg::DrawBarAndText(CDC* pDC)
{
    const UCHAR * pBarData;
    const BITMAPINFO * pBarInfo;
    if (!m_pDevice->GetOutputColorBardata(&pBarData, &pBarInfo))	//获取颜色条数据
    {
        return;
    }

    const struct_CamInfo* pCamInfo = m_pDevice->GetCamInfo();
    int w = pCamInfo->intVideoWidth;
    int h = pCamInfo->intVideoHeight;
    ...
    StretchDIBits(pDC->GetSafeHdc(), xDest, yDest, wDest, hDest, 0, 0, m_intBarW, m_intBarH, pBarData, pBarInfo, DIB_RGB_COLORS, SRCCOPY);//画颜色条

    struct_FixPara param;
    m_pDevice->GetFixPara(&param);

    const struct_State* pState = m_pDevice->GetFrameStatisticalData();
    ...
    CRect rt;
    CString str;
    UINT xPos, yPos, cx, cy;
    //画最高温刻度
    m_pDevice->ConvertPos2XY(pState->intPosMax, &xPos, &yPos);
    int intMaxTemp = m_bEnableCorrect ?
        m_pDevice->FixTemperature(pState->intMaxTemperature, param.fEmissivity, xPos, yPos) : pState->intMaxTemperature;
    str.Format(_T("%.1f"), intMaxTemp * 0.001f);
    cx = pDC->GetTextExtent(str).cx;
    cy = pDC->GetTextExtent(str).cy;
    ...
    //画最低温刻度
    m_pDevice->ConvertPos2XY(pState->intPosMin, &xPos, &yPos);
    int intMinTemp = m_bEnableCorrect ?
        m_pDevice->FixTemperature(pState->intMinTemperature, param.fEmissivity, xPos, yPos) : pState->intMinTemperature;
    str.Format(_T("%.1f"), intMinTemp * 0.001f);
    cx = pDC->GetTextExtent(str).cx;
    cy = pDC->GetTextExtent(str).cy;

    rt.left = xDest - cx - 1;
    rt.right = rt.left + cx;
    rt.top = yDest + hDest - cy / 2;
    rt.bottom = rt.top + cy;

    pDC->DrawText(str, &rt, DT_LEFT | DT_TOP);

    //画平均温刻度
    yPos = pCamInfo->intFPAHeight / 2;
    xPos = pCamInfo->intFPAWidth / 2;
    int intAveTemp = m_bEnableCorrect ?
        m_pDevice->FixTemperature(pState->intAveTemperature, param.fEmissivity, xPos, yPos) : pState->intAveTemperature;
    str.Format(_T("%.1f"), intAveTemp * 0.001f);
    cx = pDC->GetTextExtent(str).cx;
    cy = pDC->GetTextExtent(str).cy;

    int hAve = (intMinTemp == intMaxTemp)? 0 : hDest * (intAveTemp - intMinTemp) / (intMaxTemp - intMinTemp);

    rt.left = xDest - cx - 1;
    rt.right = rt.left + cx;
    rt.top = yDest + hDest - hAve - cy / 2;
    rt.bottom = rt.top + cy;

    pDC->DrawText(str, &rt, DT_LEFT | DT_TOP);
}

void CThermoGroupSampleDlg::DrawMouseTemp(CDC* pDC)
{
    const struct_CamInfo* pCamInfo = m_pDevice->GetCamInfo();
    int w = pCamInfo->intVideoWidth;
    int h = pCamInfo->intVideoHeight;

    if (m_ptMouse.x < m_intImageOffsetX || m_ptMouse.x >= m_intImageOffsetX + w ||
        m_ptMouse.y < m_intImageOffsetY || m_ptMouse.y >= m_intImageOffsetY + h)
    {
        return;
    }


    int intFPAx = 0, intFPAy = 0;
    Client2Fpa(m_ptMouse, intFPAx, intFPAy);

    int intTemp = m_pDevice->GetTemperatureProbe(intFPAx, intFPAy, 1);//获取指定位置温度
    ...
    if (m_bEnableCorrect)
    {
        intTemp = m_pDevice->FixTemperature(intTemp, param.fEmissivity, intFPAx, intFPAy);//修正温度
    }

    CString str;
    str.Format(_T("%.1f"), intTemp * 0.001f);

    int cx = pDC->GetTextExtent(str).cx;
    int cy = pDC->GetTextExtent(str).cy;

    int x = m_ptMouse.x - m_intImageOffsetX;
    int y = m_ptMouse.y - m_intImageOffsetY - cy;//默认右上
    if (y < 0)//处于上边沿
    {
        y += 20;
    ...
    COLORREF oldTextClr = pDC->SetTextColor(RGB(0, 0, 0));
    int oldMode = pDC->SetBkMode(OPAQUE);

    CRect rtShow(x, y, x + cx, y + cy);
    pDC->DrawText(str, &rtShow, DT_LEFT | DT_TOP);

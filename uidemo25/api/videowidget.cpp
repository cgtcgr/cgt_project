  ...
  m_currentPoint(-1, -1)
{
    ...
    connect(this, SIGNAL(newFrameComing(int)), this,
            SLOT(process(int)));
}
...
bool VideoWidget::linkCamera(QString ip)
{
    QLOG_INFO()<<"VideoWidget::linkCamera"<<ip;
    if (!m_magDev.IsInitialized())
    {
       QLOG_INFO()<<"m_magDev.IsInitialized() fail";
        return false;
    }
    //ROOM1 = Room1, 200.120.72.166, 6060,200.120.72.167, 3777, 200.120.72.168, xtkj66666,200.120.72.170
    if (!m_magDev.LinkCamera(ip.toStdString().c_str()))
    {
        QLOG_INFO()<<"m_magDev.LinkCamera() fail";
        return false;
    }
    return true;
    //ROOM1 = Room1, 200.120.72.166, 6060,200.120.72.167, 3777, 200.120.72.168, xtkj66666,200.120.72.170
...
void VideoWidget::process(int intCameraTemperature)
{
    Q_UNUSED(intCameraTemperature);
   // QLOG_INFO()<<"VideoWidget::process"<<intCameraTemperature;
    ...
    if (bVideoStream) {
        QImage image1(caminfo->intVideoWidth, caminfo->intVideoHeight,
                     QImage::Format_RGB888);
        UCHAR *pImgData = image1.bits();
        const UCHAR *pEnd =
...
void VideoWidget::drawTem(QPainter *painter)
{

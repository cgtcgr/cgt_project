...
bool XlsxColor::saveToXml(QXmlStreamWriter &writer, const QString &node) const
{
    if (!node.isEmpty())
        ...
        writer.writeEmptyElement(QString("color"));

    if (val.userType() == qMetaTypeId<QColor>()) {
        writer.writeAttribute(QString("rgb"), XlsxColor::toARGBString(val.value<QColor>()));
    } else if (val.userType() == QMetaType::QStringList) {
        QStringList themes = val.toStringList();
        writer.writeAttribute(QString("theme"), themes[0]);
        if (!themes[1].isEmpty())
            writer.writeAttribute(QString("tint"), themes[1]);
    } else if (val.userType() == QMetaType::Int) {
        writer.writeAttribute(QString("indexed"), val.toString());
    } else {
        writer.writeAttribute(QString("auto"), QString("1"));
    }
